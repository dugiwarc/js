...
(function (global) {
	...
	SQLParser.prototype = {
		...
		parse: (function () {
			...
			// Table column
			var tc = p.seq(table, dot, column).then(function (res) {
				return {
					table: res[0],
					column: res[2],
				};
			...
			// List of fields
			var listOfFields = p.any(p.txt("*"), p.rep(tc, p.seq(wso, comma, wso)));

			// Equality of fields
			var fieldEquality = p
				.seq(tc, wso, p.txt("="), wso, tc)
				.then(function (res) {
					return {
						left: res[0],
						right: res[4],
			...
			// Comparison value
			var val = p.any(str, num, bool, nul, tc);

			// Selection section
			var SELECT_EXP = p
				.seq(SELECT, ws, listOfFields, ws, FROM, ws, table, wso)
				.then(function (res) {
					return {
						select: {
							fields: res[2],
			...
			// Join section
			var JOIN_EXP = p
				.seq(JOIN, ws, table, ws, ON, ws, fieldEquality, wso)
				.then(function (res) {
					return {
						join: {
			...
			var COMP_EXP = p.seq(val, wso, comparison, wso, val).then(function (res) {
				return {
					left: res[0],
					right: res[4],
					type: res[2],
			...
			// Where section
			var WHERE_EXP = p.seq(WHERE, ws, COMP_EXP, wso).then(function (res) {
				return {
					where: res[2],
				};
			});
	...
	SQLEngine.prototype = {
		...
		_filter: function (where) {
			...
			function makeValueFn(expressionOperand) {
				...
				return function (row) {
					return field ? row[field] : expressionOperand;
				};
			}
